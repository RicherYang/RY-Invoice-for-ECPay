...
final class RY_IFECPAY_WC_Invoice_Basic
{
    private static ?self $_instance = null;

    public static function instance(): RY_IFECPAY_WC_Invoice_Basic
    {
    ...
    public function add_invoice_info($fields)
    {
        $fields['invoice'] = [
            ...
            'invoice_carruer_type' => [
                ...
                'options' => [
                    'ecpay_host' => ry_ifecpay_carruer_type_to_name('ecpay_host') . __(' (send paper when win)', 'ry-invoice-for-ecpay'),
        ...
        if (RY_IFECPAY::get_option('move_billing_company', 'no') === 'yes') {
            unset($fields['billing']['billing_company']);
            $fields['invoice']['invoice_company_name'] = [
                'label' => __('Company name', 'ry-invoice-for-ecpay'),
                'required' => true,
                'priority' => 30,
            ];
        }
    ...
    {
        $asset_info = include RY_IFECPAY_PLUGIN_DIR . 'assets/wc-checkout.asset.php';
        wp_enqueue_script('ry-invoice-wc-checkout', RY_IFECPAY_PLUGIN_URL . 'assets/wc-checkout.js', $asset_info['dependencies'], $asset_info['version'], true);

        $args = [
            'checkout' => $checkout,
        ];
        wc_get_template('checkout/form-invoice.php', $args, '', RY_IFECPAY_PLUGIN_DIR . 'woocommerce/templates/');
    }
                    ...
                    case 'MOICA':// 自然人憑證
                        if (!empty($data['invoice_carruer_no'])) {
                            if (!preg_match('/^[A-Z]{2}\d{14}$/', $data['invoice_carruer_no'])) {
                                $errors->add('validation', __('Invalid carruer number', 'ry-invoice-for-ecpay'));
                            }
                        }
                        break;
            ...
            case 'donate': // 愛心碼
                if (!empty($data['invoice_donate_no'])) {
                    if (!preg_match('/^[0-9]{3,7}$/', $data['invoice_donate_no'])) {
                        $errors->add('validation', __('Invalid donate number', 'ry-invoice-for-ecpay'));
                    }
                }
                break;
            case 'company':// 公司
                if (!empty($data['invoice_no'])) {
                    ...
                    if (!$valid) {
                        $errors->add('validation', __('Invalid tax ID number', 'ry-invoice-for-ecpay'));
                    }
                }
                break;
        }
    ...
    public function show_invoice_info($order)
    {
        ...
        if ($invoice_number) {
            if (preg_match('/^[A-Z]{2}[0-9]{8}$/', $invoice_number)) {
                $invoice_info[] = [
                    'key' => 'invoice-number',
                    'name' => __('Invoice number', 'ry-invoice-for-ecpay'),
                    'value' => $invoice_number,
                ];
                $invoice_info[] = [
                    'key' => 'invoice-random-number',
                    'name' => __('Invoice random number', 'ry-invoice-for-ecpay'),
                    'value' => $order->get_meta('_invoice_random_number'),
                ];
                $invoice_info[] = [
                    'key' => 'invoice-date',
                    'name' => __('Invoice date', 'ry-invoice-for-ecpay'),
                ...
                switch ($invoice_number) {
                    ...
                    case 'zero':
                        $invoice_info[] = [
                            'key' => 'zero-info',
                            'name' => __('Zero total fee without invoice', 'ry-invoice-for-ecpay'),
                            'value' => '',
                        ];
                        break;
                    case 'negative':
                        $invoice_info[] = [
                            'key' => 'negative-info',
                            'name' => __('Negative total fee can\'t invoice', 'ry-invoice-for-ecpay'),
                            'value' => '',
                        ];
                        break;
                }
        ...
        $invoice_info[] = [
            'key' => 'invoice-type',
            'name' => __('Invoice type', 'ry-invoice-for-ecpay'),
            'value' => ry_ifecpay_invoice_type_to_name($invoice_type),
        ];

        switch ($invoice_type) {
            case 'personal':
                $key = count($invoice_info) - 1;
                $invoice_info[$key]['value'] .= ' (' . ry_ifecpay_carruer_type_to_name($carruer_type) . ')';
                if (in_array($carruer_type, ['MOICA', 'phone_barcode'])) {
                    $invoice_info[] = [
                        'key' => 'carruer-number',
                        'name' => __('Carruer number', 'ry-invoice-for-ecpay'),
            ...
            case 'company':
                $invoice_info[] = [
                    'key' => 'tax-id-number',
                    'name' => __('Tax ID number', 'ry-invoice-for-ecpay'),
            ...
            case 'donate':
                $invoice_info[] = [
                    'key' => 'donate-number',
                    'name' => __('Donate number', 'ry-invoice-for-ecpay'),
        ...
        wc_get_template('order/order-invoice-info.php', $args, '', RY_IFECPAY_PLUGIN_DIR . 'woocommerce/templates/');
    }
}
