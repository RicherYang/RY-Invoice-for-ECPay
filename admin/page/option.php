...
final class RY_IFECPAY_Admin_Page_Option extends RY_Abstract_Admin_Page
{
    ...
    protected function do_init(): void
    {
        ...
        if ($_wp_menu_nopriv) {
            ...
            $_wp_real_parent_file['ry-invoice-ecpay-option'] = RY_IFECPAY_Admin::instance()->main_slug;
    ...
    public function output_page(): void
    {
        ...
        include RY_IFECPAY_PLUGIN_DIR . 'admin/page/html/option.php';
        submit_button();
        echo '</form>';
    ...
    public function do_admin_action(string $action): void
    {
        ...
        if (!wp_verify_nonce($_POST['_wpnonce'] ?? '', 'ry-invoice-ecpay-option')) {
            wp_die('Invalid nonce');
        }

        $log = sanitize_locale_name($_POST['log'] ?? '') === 'yes' ? 'yes' : 'no';
        RY_IFECPAY::update_option('log', $log);
        $api_info = [
...
RY_IFECPAY_Admin_Page_Option::init_menu();
