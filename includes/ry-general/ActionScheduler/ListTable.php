...
final class ListTable extends \ActionScheduler_ListTable
{
    ...
    protected function get_schedule_display_string(\ActionScheduler_Schedule $schedule)
    {
        ...
        if (is_a($schedule, 'ActionScheduler_NullSchedule')) {
            return __('async', 'ry-invoice-for-ecpay');
        }
        ...
        if (gmdate('U') > $next_timestamp) {
            ...
            $schedule_display_string .= sprintf(__(' (%s ago)', 'ry-invoice-for-ecpay'), human_time_diff(gmdate('U'), $next_timestamp));
        } else {
            /* translators: %s: date interval */
            $schedule_display_string .= sprintf(__(' (%s)', 'ry-invoice-for-ecpay'), human_time_diff($next_timestamp, gmdate('U')));
        }
