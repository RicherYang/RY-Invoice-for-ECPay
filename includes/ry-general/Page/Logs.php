...
final class Logs extends AbstractAdminPage
{
    ...
    {
        $menu_list[] = [
            'name' => __('Logs', 'ry-invoice-for-ecpay'),
            'slug' => 'ry-logs',
    ...
    public function output_page(): void
    {
        ...
        echo '<div class="wrap">';
        echo '<h1 class="wp-heading">' . esc_html__('Logs', 'ry-invoice-for-ecpay') . '</h1>';
        if ($current_group === '') {
            echo '<p>' . esc_html__('No logs found.', 'ry-invoice-for-ecpay') . '</p>';
            echo '</div>';
            return;
        }
            ...
            if (str_starts_with($current_file, $this->log_path)) {
                ...
                if (sanitize_key($_GET['action2'] ?? '') === 'delete') {
                    if (wp_delete_file($current_file)) {
                        $this->add_notice('success', __('Delete successful.', 'ry-invoice-for-ecpay'));
                        $current_log = '';
                        ...
                        $this->add_notice('error', __('Delete failed.', 'ry-invoice-for-ecpay'));
                    }
                }
            }
        }
